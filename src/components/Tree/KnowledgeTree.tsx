            ...
            {knowledgeData.branches.map((branch) => (
              <div key={branch.id} className="flex flex-col items-center">
                ...
                <div className="relative flex flex-col items-center w-full">
                  <h3 className="text-base font-bold text-deep mb-4 bg-white px-4 py-1.5 rounded-full shadow-sm border border-gray-100 z-10 text-center">
                    {branch.name}
                  </h3>
                  {/* Vertical connector from header to nodes */}
                  <div className="w-px h-4 bg-gray-200 -mt-1 mb-0" />
                </div>
                ...
                <div className="flex flex-col gap-4 w-full">
                  {branch.nodes.map((node, nodeIdx) => {
                    ...
                    const isLocked = !isUnlocked && node.status !== 'completed';
                    ...
                    return (
                      <div key={node.id} className="flex flex-col items-center">
                        ...
                        <motion.div
                          ...
                          className={`relative w-full p-4 rounded-xl shadow-sm border flex flex-col items-center text-center cursor-pointer transition-all ${
                            isCompleted
                              ? 'bg-primary border-primary text-white shadow-primary/30 shadow-md'
                              : 'bg-gray-50 border-gray-200 opacity-60 grayscale'
                          }`}
                        >
                          {isLocked && <Lock size={14} className="absolute top-2 right-2 opacity-40" />}
